...
bool menu() {
	...
	string menu_inp;
	cout << "Welcome to Alpha 1.1 of Cardno!" << endl;
...
int main() {
	//In case when run in a terminal
	cout << string(50, '\n');

	//Set title for console window (remove if porting versions to *nix)
	SetConsoleTitle(L"Cardno");
	...
	while (loopg) {
		...
		if (ct == true) {
			...
			if (rand_add == rand_match) {
				...
				cout << e_cards[chs - 1] << " VS " << p_cards[p_choose - 1] << endl;

				if (e_cards[chs - 1] > p_cards[p_choose - 1]) {
					e_cards[chs - 1] = rand_card;
					cout << "Your opponent takes the card!" << endl;
				}
				else if (e_cards[chs - 1] < p_cards[p_choose - 1]) {
					...
					string er_wild = "Unknown error with chs and p_choose";
					error_rep(er_wild);
				}
			}
		}
		...
		cout << "1: " << p_cards[0] << "  2: " << p_cards[1] << "  3: " << p_cards[2] << "  4: " << p_cards[3] << "  5: " << p_cards[4] << "  6: Exit" << endl;
		cout << "--------------" << endl;
		...
		chs = rand() % 4 + 0;
		e_choose = chs;
		cout << "Your opponent chose: Card number " << e_choose + 1 << ", which is the card: " << e_cards[e_choose] << "." << endl;
		cout << "Do you wish to trade? (y/n) ";
		
		cin >> choose;	
		if (choose == "y" || choose == "Y") {
			
			int card_ol = e_cards[e_choose];
			int p_ch = p_choose;
			e_cards[e_choose] = p_cards[p_ch]; 
			p_cards[p_ch] = card_ol;
...
void wilo(bool cha) {
	if (cha == false) {
		...
		cout << "Type a letter then press enter to continue. . .";
		cin >> winpause;
	}
	else if (cha == true) {
		string losepause;
